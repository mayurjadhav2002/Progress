                                ...
                                <Combobox value={group.name} onChange={HandleGroup}>
                                    <div className='relative'>
                                        <div className="absolute inset-y-0 start-0 flex items-center ps-3.5 cursor-pointer">
                                            <input type="color" className='w-8 h-8 rounded-full border-none
                                            bg-transparent
                                            ' value={group.color} onChange={(e) => setGroup({ ...group, color: e.target.value })} />
                                        </div>
                                        <Combobox.Input value={group.name} onChange={(event) => { props.setQuery(event.target.value); HandleGroup(event.target.value) }}
                                            ...
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg ps-14 focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 
                                             dark:bg-gray-900 dark:border-gray-800 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        />
                                    </div>
                                    <Combobox.Options className={'overflow-y-scroll max-h-32 bg-gray-100 dark:bg-dark dark:border-2 dark:border-[#1c1c1c] rounded-e-lg custom-scrollbar'}>
                                        {folders && folders.length > 0 && folders?.map((person) => (
                                            <Combobox.Option key={person} value={person} className="px-5 py-2 text-black hover:text-primary hover:bg-blue-100
                                            dark:hover:bg-dark dark:text-white 
                                            cursor-pointer">
                                                {person}
                                            </Combobox.Option>
                                        ))}
                    ...
                    <TabPanel key={'delete'} value={'delete'}>
                        <div>
                            ...
                            <div className="relative flex w-full mt-2">
                                <Input type="text" label="Enter 'DELETE' "
                                    // onChange={}
                                    className="pr-20"
                                    variant='standard'
                                    containerProps={{
                                ...
                                <Button
                                    size="sm"
                                    variant="destructive"                                 // color={email ? "gray" : "blue-gray"}
                                    // disabled={!email}
                            ...
                            <span
                                className="mt-5 flex items-start gap-1 font-normal text-gray-500 text-xs"
                            >
                               
                             <FaInfoCircle/>   Once you delete Document you cannot able to recover it again in future, so reconsider again
                            </span>
                        </div>
                    </TabPanel>
